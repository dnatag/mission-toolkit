...
// preCommitMockGitClient simulates pre-commit hooks modifying files
type preCommitMockGitClient struct {
	internalgit.GitClient
	fs             afero.Fs
	repo           *git.Repository
	scopeFile      string
	modifyCount    int
...
func (m *preCommitMockGitClient) Commit(message string) (string, error) {
	hash, err := m.GitClient.Commit(message)
	if err != nil {
		return "", err
	}
	// Simulate pre-commit hook modifying files (up to maxModifyCount times)
	if m.modifyCount < m.maxModifyCount {
		m.modifyCount++
		// Modify the file in afero fs to simulate pre-commit hook
		afero.WriteFile(m.fs, m.scopeFile, []byte(fmt.Sprintf("modified-%d", m.modifyCount)), 0644)
	}
	return hash, nil
}

func (m *preCommitMockGitClient) GetUnstagedFiles() ([]string, error) {
	// Check if afero fs differs from git worktree (simulates pre-commit modification)
	aferoContent, err := afero.ReadFile(m.fs, m.scopeFile)
	if err != nil {
		return nil, nil
	}
	wt, _ := m.repo.Worktree()
	f, err := wt.Filesystem.Open(m.scopeFile)
	if err != nil {
		return []string{m.scopeFile}, nil
	}
	defer f.Close()
	wtContent := make([]byte, 1024)
	n, _ := f.Read(wtContent)
	if string(aferoContent) != string(wtContent[:n]) {
		return []string{m.scopeFile}, nil
	}
	return nil, nil
}

func TestService_Create_PreCommitHookRetry(t *testing.T) {
	fs, repo := setupTestRepo(t)

	missionID := "test-precommit"
	scopeFile := "precommit.txt"
	createMissionFile(t, fs, missionID, []string{scopeFile})

	err := afero.WriteFile(fs, scopeFile, []byte("content"), 0644)
	require.NoError(t, err)

	baseClient := internalgit.NewMemGitClient(repo, fs)
	mockClient := &preCommitMockGitClient{
	...
	svc := NewServiceWithGit(fs, ".mission", mockClient)
...
func TestService_Create_PreCommitHookExceedsRetries(t *testing.T) {
	fs, repo := setupTestRepo(t)

	missionID := "test-precommit-fail"
	scopeFile := "precommit-fail.txt"
	createMissionFile(t, fs, missionID, []string{scopeFile})
	...
	baseClient := internalgit.NewMemGitClient(repo, fs)
	// Always modify file after commit (never stops)
	mockClient := &preCommitMockGitClient{
		GitClient:      baseClient,
		fs:             fs,
		repo:           repo,
		scopeFile:      scopeFile,
		maxModifyCount: 10, // More than max retries
	}
	svc := NewServiceWithGit(fs, ".mission", mockClient)
	...
	_, err = svc.Create(missionID)
	require.Error(t, err)
	require.Contains(t, err.Error(), "pre-commit hooks modified files after 3 retries")
}
