...
func (p *Provider) reconcile() {
	...
	// If the snapshot was generated by Beads, it's the source of truth —
	// don't re-import its contents as rogue edits.
	if strings.Contains(string(data), "MANAGED BY BEADS") {
		os.WriteFile(p.hashPath, []byte(currentHash), 0600)
		return
	}
